...
CFLAGS += -I $(STD_PERIPH_LIB)/STM32F4xx_StdPeriph_Driver/inc
#CFLAGS += -I $(STD_PERIPH_LIB)/STM32_USB-FS-Device_Driver/inc
#CFLAGS += -include $(STD_PERIPH_LIB)/stm32f4xx_conf.h

STARTUP = src/startup_stm32f4xx.s # add startup file to build
...
$(PROJ_NAME).elf: $(OBJS)
	$(CC) $(CFLAGS) $(LDFLAGS) $^ -o $@ $(STARTUP) -L$(STD_PERIPH_LIB) -lstm32f4 -L$(LDSCRIPT_INC) -Tstm32f407_flash.ld
